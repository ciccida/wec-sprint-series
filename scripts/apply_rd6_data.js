...
const PARSED_JSON = 'rd6_parsed.json';
...
async function run() {
    const rd6Data = JSON.parse(fs.readFileSync(PARSED_JSON, 'utf8'));

    // 1. Update raceResults
    raceResults["Vol3"]["6"] = rd6Data;
    ...
    const hypercar = rd6Data.filter(r => r.category === 'Hypercar');
    const lmgt3 = rd6Data.filter(r => r.category === 'LMGT3');
    ...
    function assignPoints(raceList) {
        raceList.forEach((r, i) => {
            ...
            if (existingDrivers.has(normName)) {
                ...
                entry.points[5] = points; // Index 5 is Rd6
            } else if (points > 0) {
                ...
                maxId++;
                    const newEntry = {
                        ...
                        rank: 99,
                        name: r.driver,
                        points: [null, null, null, null, null, points, null, null]
                    };
