...
const PARSED_JSON = 'rd7_parsed.json';
...
async function run() {
    const rd7Data = JSON.parse(fs.readFileSync(PARSED_JSON, 'utf8'));

    // 1. Update raceResults
    raceResults["Vol3"]["7"] = rd7Data;
    ...
    const hypercar = rd7Data.filter(r => r.category === 'Hypercar');
    const lmgt3 = rd7Data.filter(r => r.category === 'LMGT3');
    ...
    function assignPoints(raceList) {
        raceList.forEach((r, i) => {
            ...
            if (existingDrivers.has(normName)) {
                ...
                entry.points[6] = points; // Index 6 is Rd7
            } else if (points > 0) {
                ...
                maxId++;
                    const newEntry = {
                        ...
                        rank: 99,
                        name: r.driver,
                        points: [null, null, null, null, null, null, points, null]
                    };
