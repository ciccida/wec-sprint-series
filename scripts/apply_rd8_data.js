...
const PARSED_JSON = 'rd8_parsed.json';
...
async function run() {
    const rd8Data = JSON.parse(fs.readFileSync(PARSED_JSON, 'utf8'));

    // 1. Update raceResults
    raceResults["Vol3"]["8"] = rd8Data;
    ...
    const hypercar = rd8Data.filter(r => r.category === 'Hypercar');
    const lmgt3 = rd8Data.filter(r => r.category === 'LMGT3');
    ...
    function assignPoints(raceList) {
        raceList.forEach((r, i) => {
            ...
            if (existingDrivers.has(normName)) {
                ...
                entry.points[7] = points; // Index 7 is Rd8
            } else if (points > 0) {
                ...
                maxId++;
                    const newEntry = {
                        ...
                        rank: 99,
                        name: r.driver,
                        points: [null, null, null, null, null, null, null, points]
                    };
